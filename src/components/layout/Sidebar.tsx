...
import {
  LayoutDashboard, Users, Kanban, Radar, Mail, FileText, CheckSquare, Settings, Flame,
} from 'lucide-react';
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex items-center justify-between px-2.5 py-2 rounded-lg text-[13px] font-medium transition ${
                  isActive ? 'bg-blue-600/15 text-blue-400 border border-blue-500/25' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900/70 border border-transparent'
                }`}>
                ...
                {item.badge && (
                  <span className="text-[9px] font-semibold bg-blue-500/20 text-blue-300 border border-blue-500/30 px-1.5 py-0.5 rounded-full">{item.badge}</span>
                )}
              </Link>
            );
          })}
